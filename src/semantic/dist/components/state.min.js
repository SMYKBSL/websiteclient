  ...
  (t =
    ...
    (e.fn.state = function(t) {
      var i,
        o = e(this),
        s = o.selector || '',
        c = ('ontouchstart' in n.documentElement, new Date().getTime()),
        r = [],
        l = arguments[0],
        u = 'string' == typeof l,
          ...
          (n = {
            initialize: function() {
              n.verbose('Initializing module'),
                ...
                f.context && '' !== s
                  ? e(f.context)
                      .on(s, 'mouseenter' + m, n.change.text)
                      .on(s, 'mouseleave' + m, n.reset.text)
                      .on(s, 'click' + m, n.toggle.state)
                  : y
                      .on('mouseenter' + m, n.change.text)
                      .on('mouseleave' + m, n.reset.text)
                      .on('click' + m, n.toggle.state),
                n.instantiate();
            },
            ...
            toggle: {
              state: function() {
                ...
                if (n.allows('active') && n.is.enabled()) {
                  if ((n.refresh(), e.fn.api !== a))
                    if (
                      ((t = y.api('get request')), (i = y.api('was cancelled')))
                    )
            ...
            listenTo: function(t) {
              n.debug('API request detected, waiting for state signal', t),
                t &&
                  (p.loading && n.update.text(p.loading),
            ...
            change: {
              ...
              text: function() {
                n.is.textEnabled() &&
                  (n.is.disabled()
                    ? (n.verbose('Changing text to disabled text', p.hover),
                      n.update.text(p.disabled))
                    : n.is.active()
                      ? p.hover
                        ? (n.verbose('Changing text to hover text', p.hover),
                          n.update.text(p.hover))
                        : p.deactivate &&
                          (n.verbose(
                            'Changing text to deactivating text',
                            p.deactivate
                          ),
                          n.update.text(p.deactivate))
                      : p.hover
                        ? (n.verbose('Changing text to hover text', p.hover),
                          n.update.text(p.hover))
                        : p.activate &&
            ...
            deactivate: function() {
              f.deactivateTest.call(w) &&
                ...
                n.update.text(p.inactive),
                f.onDeactivate.call(w));
            },
            sync: function() {
              n.verbose('Syncing other buttons to current state'),
                n.is.active()
                  ? o.not(y).state('activate')
                  : o.not(y).state('deactivate');
            },
            get: {
              text: function() {
                return f.selector.text
                  ? y.find(f.selector.text).text()
                  : y.html();
              },
              textFor: function(e) {
                return p[e] || !1;
            ...
            reset: {
              text: function() {
                var e = p.active || y.data(g.storedText),
                  t = p.inactive || y.data(g.storedText);
                n.is.textEnabled() &&
                  (n.is.active() && e
                    ...
                    var s =
                      i != c
                        ? o +
                          ...
                          t[i + 1].slice(1)
                        : t;
                    if (e.isPlainObject(u[s]) && i != c) u = u[s];
                    else {
                      if (u[s] !== a) return (r = u[s]), !1;
                      if (!e.isPlainObject(u[o]) || i == c)
                        return u[o] !== a
                          ? ((r = u[o]), !1)
                          : (n.error(v.method, t), !1);
                      u = u[o];
                    }
                  })),
                e.isFunction(r) ? (l = r.apply(s, o)) : r !== a && (l = r),
                e.isArray(i)
                  ? i.push(l)
                  : i !== a
                    ? (i = [i, l])
            ...
            u
              ? (C === a && n.initialize(), n.invoke(l))
              : (C !== a && C.invoke('destroy'), n.initialize());
        }),
        i !== a ? i : this
      );
    }),
