...
!(function(e, i, n, t) {
  'use strict';
  (i =
    'undefined' != typeof i && i.Math == Math
      ? i
      ...
      return (
        s.each(function() {
          var s,
            ...
            H = e(this),
            M = e(T.context),
            D = H.children(x.sidebar),
            j = M.children(x.fixed),
            R = M.children(x.pusher),
            z = this,
            B = H.data(E);
          (w = {
            initialize: function() {
              w.debug('Initializing sidebar', o),
                w.create.id(),
                (k = w.get.transitionEvent()),
                w.is.ios() && w.set.ios(),
                T.delaySetup ? v(w.setup.layout) : w.setup.layout(),
                v(function() {
                  w.setup.cache();
                }),
                w.instantiate();
            ...
            destroy: function() {
              w.verbose('Destroying previous module for', H),
                H.off(P).removeData(E),
            ...
            event: {
              clickaway: function(e) {
                var i = R.find(e.target).length > 0 || R.is(e.target),
                  n = M.is(e.target);
                i && (w.verbose('User clicked on dimmed page'), w.hide()),
                  n &&
                    (w.verbose(
              ...
              scroll: function(i) {
                0 === e(i.target).closest(x.sidebar).length &&
                  i.preventDefault();
              }
            },
            bind: {
              clickaway: function() {
                w.verbose('Adding clickaway events to context', M),
                  T.closable &&
                    M.on('click' + p, w.event.clickaway).on(
                      'touchend' + p,
                      w.event.clickaway
                    );
              },
              scrollLock: function() {
                T.scrollLock &&
                  ...
                  a.on('DOMMouseScroll' + p, w.event.scroll)),
            ...
            unbind: {
              clickaway: function() {
                w.verbose('Removing clickaway events from context', M),
                  M.off(p);
              },
              scrollLock: function() {
            ...
            add: {
              inlineCSS: function() {
                var i,
                  ...
                  o = w.is.rtl(),
                  r = w.get.direction(),
                  a = { left: n, right: -n, top: t, bottom: -t };
                o &&
              ...
              layout: function() {
                0 === M.children(x.pusher).length &&
                  ...
                  M.children()
                    ...
                    .wrapAll(R),
                  w.refresh()),
                  (0 !== H.nextAll(x.pusher).length &&
                    H.nextAll(x.pusher)[0] === R[0]) ||
                    (w.debug('Moved sidebar to correct parent element'),
                    w.error(O.movedSidebar, z),
                    H.detach().prependTo(M),
                    w.refresh()),
                  w.clear.cache(),
                  w.set.pushable(),
                  w.set.direction();
            ...
            attachEvents: function(i, n) {
              ...
              (n = e.isFunction(w[n]) ? w[n] : w.toggle),
                t.length > 0
                  ? (w.debug('Attaching sidebar events to element', i, n),
                    t.on('click' + P, n))
                  : w.error(O.notFound, i);
            },
            show: function(i) {
              if (((i = e.isFunction(i) ? i : function() {}), w.is.hidden())) {
                if (
                  (w.refreshSidebars(),
                  T.overlay && (w.error(O.overlay), (T.transition = 'overlay')),
                  w.refresh(),
                  w.othersActive())
                )
                  if (
                    (w.debug('Other sidebars currently visible'), T.exclusive)
                  ) {
                    if ('overlay' != T.transition)
                      return void w.hideOthers(w.show);
                    w.hideOthers();
                  } else T.transition = 'overlay';
                w.pushPage(function() {
            ...
            othersAnimating: function() {
              return D.not(H).filter('.' + S.animating).length > 0;
            },
            othersVisible: function() {
              return D.not(H).filter('.' + S.visible).length > 0;
            },
            othersActive: function() {
              return w.othersVisible() || w.othersAnimating();
            },
            hideOthers: function(e) {
              var i = D.not(H).filter('.' + S.visible),
                n = i.length,
                t = 0;
              (e = e || function() {}),
            ...
            pushPage: function(i) {
              var n,
                ...
                o,
                r = w.get.transition(),
                s = 'overlay' === r || w.othersActive() ? H : R;
              (i = e.isFunction(i) ? i : function() {}),
                'scale down' == T.transition && w.scrollToTop(),
                w.set.transition(r),
                w.repaint(),
                (n = function() {
                  w.bind.clickaway(),
                ...
                s.on(k + p, o),
                v(n),
                T.dimPage && !w.othersVisible() && v(t);
            },
            pullPage: function(i) {
              var n,
                t,
                o = w.get.transition(),
                r = 'overlay' == o || w.othersActive() ? H : R;
              (i = e.isFunction(i) ? i : function() {}),
                ...
                (n = function() {
                  w.set.transition(o),
                    w.set.animating(),
                    w.remove.visible(),
                    T.dimPage && !w.othersVisible() && R.removeClass(S.dimmed);
                }),
                (t = function(e) {
                  e.target == r[0] &&
                    (r.off(k + p, t),
                    ...
                    w.remove.transition(),
                    w.remove.inlineCSS(),
                    ('scale down' == o || (T.returnScroll && w.is.mobile())) &&
                      w.scrollBack(),
                    i.call(z));
                }),
                r.off(k + p),
            ...
            remove: {
              inlineCSS: function() {
                w.debug('Removing inline css styles', s),
              ...
              pushed: function() {
                M.removeClass(S.pushed);
              },
              pushable: function() {
                M.removeClass(S.pushable);
              },
              active: function() {
            ...
            get: {
              ...
              transition: function() {
                ...
                return (
                  (e = w.is.mobile()
                    ? 'auto' == T.mobileTransition
                      ? T.defaultTransition.mobile[i]
                      : T.mobileTransition
                    : 'auto' == T.transition
                      ? T.defaultTransition.computer[i]
                      : T.transition),
            ...
            is: {
              ie: function() {
                var e = !i.ActiveXObject && 'ActiveXObject' in i,
                  n = 'ActiveXObject' in i;
                return e || n;
              },
              ios: function() {
                var e = navigator.userAgent,
                  i = e.match(F.ios),
                  n = e.match(F.mobileChrome);
                return (
                  !(!i || n) &&
              ...
              animating: function() {
                return M.hasClass(S.animating);
              },
              rtl: function() {
                return (
                  w.cache.rtl === t &&
                    (w.cache.rtl = 'rtl' == H.css('direction')),
                  w.cache.rtl
                );
              }
            },
            setting: function(i, n) {
              if ((w.debug('Changing setting', i, n), e.isPlainObject(i)))
                e.extend(!0, T, i);
              else {
                if (n === t) return T[i];
                e.isPlainObject(T[i]) ? e.extend(!0, T[i], n) : (T[i] = n);
              }
            },
            internal: function(i, n) {
              if (e.isPlainObject(i)) e.extend(!0, w, i);
              else {
                if (n === t) return w[i];
                w[i] = n;
              }
                ...
                'string' == typeof i &&
                  ...
                  e.each(i, function(n, o) {
                    var r =
                      n != s
                        ? o +
                          i[n + 1].charAt(0).toUpperCase() +
                          i[n + 1].slice(1)
                        : i;
                    if (e.isPlainObject(c[r]) && n != s) c = c[r];
                    else {
                      if (c[r] !== t) return (a = c[r]), !1;
                      if (!e.isPlainObject(c[o]) || n == s)
                        return c[o] !== t
                          ? ((a = c[o]), !1)
                          : (w.error(O.method, i), !1);
                      c = c[o];
                    }
                  })),
                e.isFunction(a) ? (l = a.apply(o, n)) : a !== t && (l = a),
                e.isArray(r)
                  ? r.push(l)
                  : r !== t
                    ? (r = [r, l])
            ...
            m
              ? (B === t && w.initialize(), w.invoke(h))
              : (B !== t && w.invoke('destroy'), w.initialize());
        }),
        r !== t ? r : this
      );
    }),
