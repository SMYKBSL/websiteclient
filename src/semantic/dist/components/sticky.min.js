          ...
          (p = {
            initialize: function() {
              p.determineContainer(),
                p.determineContext(),
                p.verbose('Initializing sticky', b, r),
                p.save.positions(),
                p.checkErrors(),
                p.bind.events(),
                b.observeChanges && p.observeChanges(),
                p.instantiate();
            },
            ...
            destroy: function() {
              p.verbose('Destroying previous instance'),
                ...
                h && h.disconnect(),
                g && g.disconnect(),
                T.off('load' + S, p.event.load).off(
                  'resize' + S,
                  p.event.resize
                ),
                w.off('scrollchange' + S, p.event.scrollchange),
                k.removeData(y);
            },
            observeChanges: function() {
              'MutationObserver' in t &&
                ((h = new MutationObserver(p.event.documentChanged)),
                (g = new MutationObserver(p.event.changed)),
                h.observe(o, { childList: !0, subtree: !0 }),
                g.observe(P, { childList: !0, subtree: !0 }),
                g.observe(d[0], { childList: !0, subtree: !0 }),
            ...
            determineContext: function() {
              if (((d = b.context ? e(b.context) : r), 0 === d.length))
                return void p.error(C.invalidContext, b.context, k);
            },
            checkErrors: function() {
              if (
                (p.is.hidden() && p.error(C.visible, k),
                p.cache.element.height > p.cache.context.height)
              )
                return p.reset(), void p.error(C.elementSize, k);
            },
            bind: {
              events: function() {
                T.on('load' + S, p.event.load).on('resize' + S, p.event.resize),
                  w
                    .off('scroll' + S)
                    .on('scroll' + S, p.event.scroll)
                    .on('scrollchange' + S, p.event.scrollchange);
              }
            },
            event: {
              changed: function(e) {
            ...
            refresh: function(e) {
              p.reset(),
                b.context || p.determineContext(),
                e && p.determineContainer(),
                p.save.positions(),
                p.stick(),
                b.onReposition.call(P);
            },
            supports: {
              sticky: function() {
                var t = e('<div/>');
                t[0];
                return (
                  t.addClass(v.supported), t.css('position').match('sticky')
                );
              }
            },
            save: {
              ...
              positions: function() {
                var e = { height: w.height() },
                  ...
                  o = { offset: d.offset(), height: d.outerHeight() };
            ...
            get: {
              ...
              scrollChange: function(e) {
                return (
                  (e = e || w.scrollTop()), p.lastScroll ? e - p.lastScroll : 0
                );
              },
              ...
              elementScroll: function(e) {
                ...
                var t = p.cache.element,
                  ...
                  r = s + n;
                return (s = p.cache.fits || r < 0 ? 0 : r > i ? i : r);
              }
            },
            remove: {
              lastScroll: function() {
            ...
            set: {
              offset: function() {
                p.verbose('Setting offset on element', b.offset),
                  k.css('margin-top', b.offset);
              },
              containerSize: function() {
                var e = r.get(0).tagName;
                'HTML' === e || 'body' == e
                  ? p.determineContainer()
                  : Math.abs(r.outerHeight() - p.cache.context.height) >
                      b.jitter &&
                    (p.debug(
                      'Context has padding, specifying exact height for container',
                      p.cache.context.height
                    ),
                    r.css({ height: p.cache.context.height }));
              },
              ...
              scroll: function(e) {
                p.debug('Setting scroll on element', e),
                  p.elementScroll != e &&
                    ...
                    p.is.bottom() && k.css('top', '').css('bottom', e));
              },
              size: function() {
            ...
            stick: function(e) {
              var t = e || w.scrollTop(),
                ...
                l = (p.get.direction(e.top),
                n ? 0 : p.get.elementScroll(e.top)),
                a = !n,
                f = 0 !== i.height;
              f &&
                ...
                'string' == typeof t &&
                  ...
                  e.each(t, function(o, i) {
                    ...
                    if (e.isPlainObject(a[s]) && o != r) a = a[s];
                      ...
                      if (!e.isPlainObject(a[i]) || o == r)
                        return a[i] !== n && ((c = a[i]), !1);
                      a = a[i];
                    }
                  })),
                e.isFunction(c) ? (l = c.apply(i, o)) : c !== n && (l = c),
                e.isArray(s)
                  ? s.push(l)
                  : s !== n
                    ? (s = [s, l])
            ...
            m
              ? (z === n && p.initialize(), p.invoke(f))
              : (z !== n && z.invoke('destroy'), p.initialize());
        }),
        s !== n ? s : this
      );
    }),
