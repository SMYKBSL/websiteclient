    ...
    (e.fn.modal = function(o) {
      ...
      return (
        r.each(function() {
          ...
          (C = {
            initialize: function() {
              C.verbose('Initializing dimmer', q),
                C.create.id(),
                C.create.dimmer(),
                C.refreshModals(),
                C.bind.events(),
                F.observeChanges && C.observeChanges(),
                C.instantiate();
            },
            instantiate: function() {
              C.verbose('Storing instance of modal'), (P = C), O.data(z, P);
            },
            create: {
              dimmer: function() {
                var n = {
                    debug: F.debug,
                    dimmerName: 'modals',
                    duration: { show: F.duration, hide: F.duration }
                  },
                  ...
                  F.inverted &&
                    (i.variation =
                      i.variation !== t
                        ...
                        : 'inverted'),
                  e.fn.dimmer === t
                    ? void C.error(T.dimmer)
                    : (C.debug('Creating dimmer with settings', i),
                      (y = q.dimmer(i)),
                      F.detachable
                        ? (C.verbose(
                        ...
                        : C.set.undetached(),
                      F.blurring && y.addClass(D.blurring),
                      void (k = y.dimmer('get dimmer')))
                );
              },
              id: function() {
                (M = (Math.random().toString(16) + '000000000').substr(2, 8)),
                  (w = '.' + M),
                  C.verbose('Creating unique id for element', M);
              }
            },
            destroy: function() {
              C.verbose('Destroying previous modal'),
                O.removeData(z).off(x),
                s.off(w),
                k.off(w),
                E.off(x),
                q.dimmer('destroy');
            ...
            observeChanges: function() {
              'MutationObserver' in n &&
                ...
                S.observe(j, { childList: !0, subtree: !0 }),
            ...
            attachEvents: function(n, i) {
              var t = e(n);
              (i = e.isFunction(C[i]) ? C[i] : C.toggle),
                t.length > 0
                  ? (C.debug('Attaching modal events to element', n, i),
                    t.off(x).on('click' + x, i))
                  : C.error(T.notFound, n);
            },
            bind: {
              events: function() {
                C.verbose('Attaching events'),
                  O.on('click' + x, A.close, C.event.close)
                    .on('click' + x, A.approve, C.event.approve)
                    .on('click' + x, A.deny, C.event.deny),
                  s.on('resize' + w, C.event.resize);
              }
            },
            get: {
              id: function() {
            ...
            event: {
              approve: function() {
                return F.onApprove.call(j, e(this)) === !1
                  ? void C.verbose(
              ...
              deny: function() {
                return F.onDeny.call(j, e(this)) === !1
                  ? void C.verbose(
              ...
              click: function(n) {
                ...
                !o &&
                  ...
                  C.is.active() &&
                    (C.remove.clickaway(),
                    F.allowMultiple ? C.hide() : C.hideAll()));
              },
              ...
              keyboard: function(e) {
                ...
                n == i &&
                  (F.closable
                    ? (C.debug('Escape key pressed hiding modal'), C.hide())
                    : C.debug(
              ...
              resize: function() {
                y.dimmer('is active') && v(C.refresh);
              }
            },
            toggle: function() {
              ...
              (n = e.isFunction(n) ? n : function() {}),
                C.is.animating() || !C.is.active()
                  ? (C.showDimmer(),
                    ...
                    C.set.type(),
                    C.set.clickaway(),
                    !F.allowMultiple && C.others.active()
                      ...
                      O.transition('is supported')
                        ? (C.remove.active(),
                          O.transition({
                            ...
                            useFailSafe: !0,
                            onStart: function() {
                              C.others.active() || i || C.hideDimmer(),
                                F.keyboardShortcuts &&
                                  C.remove.keyboardShortcuts();
                            },
                            onComplete: function() {
                              F.onHidden.call(j), C.restore.focus(), n();
                            }
                          }))
                        : C.error(T.noTransition))
                    )
            ...
            showDimmer: function() {
              y.dimmer('is animating') || !y.dimmer('is active')
                ? (C.debug('Showing dimmer'), y.dimmer('show'))
            ...
            hideAll: function(n) {
              ...
              (n = e.isFunction(n) ? n : function() {}),
                i.length > 0 &&
                  ...
                  C.hideDimmer(),
                  i.modal('hide modal', n));
            },
            hideOthers: function(n) {
              var i = b.filter('.' + D.active + ', .' + D.animating);
              (n = e.isFunction(n) ? n : function() {}),
                i.length > 0 &&
                  (C.debug('Hiding other modals', b),
                  i.modal('hide modal', n, !0));
            },
            others: {
              active: function() {
            ...
            remove: {
              ...
              screenHeight: function() {
                C.debug('Removing page height'), l.css('height', '');
              },
              keyboardShortcuts: function() {
                C.verbose('Removing keyboard shortcuts'), c.off('keyup' + x);
              },
              scrolling: function() {
            ...
            cacheSizes: function() {
              ...
              (C.cache !== t && 0 === o) ||
                (C.cache = {
                  ...
                  height: o + F.offset,
                  contextHeight:
                    'body' == F.context ? e(n).height() : y.height()
                }),
                C.debug('Caching modal and container sizes', C.cache);
            },
            can: {
              fit: function() {
                return C.cache.height + 2 * F.padding < C.cache.contextHeight;
              }
            },
            is: {
              active: function() {
            ...
            set: {
              ...
              clickaway: function() {
                F.closable && k.on('click' + w, C.event.click);
              },
              screenHeight: function() {
                C.can.fit()
                  ? l.css('height', '')
                  : (C.debug(
                    ...
                    l.css('height', C.cache.height + 2 * F.padding));
              },
              active: function() {
                O.addClass(D.active);
              },
              ...
              position: function() {
                C.verbose('Centering modal on page', C.cache),
                  C.can.fit()
                    ? O.css({ top: '', marginTop: -(C.cache.height / 2) })
            ...
            setting: function(n, i) {
              if ((C.debug('Changing setting', n, i), e.isPlainObject(n)))
                e.extend(!0, F, n);
              else {
                if (i === t) return F[n];
            ...
            internal: function(n, i) {
              if (e.isPlainObject(n)) e.extend(!0, C, n);
              else {
                if (i === t) return C[n];
                C[n] = i;
              }
                ...
                'string' == typeof n &&
                  ...
                  e.each(n, function(i, o) {
                    var a =
                      i != r
                        ? o +
                          n[i + 1].charAt(0).toUpperCase() +
                          n[i + 1].slice(1)
                        : n;
                    if (e.isPlainObject(l[a]) && i != r) l = l[a];
                    else {
                      if (l[a] !== t) return (s = l[a]), !1;
                      if (!e.isPlainObject(l[o]) || i == r)
                        return l[o] !== t && ((s = l[o]), !1);
                      l = l[o];
                    }
                  })),
                e.isFunction(s) ? (c = s.apply(o, i)) : s !== t && (c = s),
                e.isArray(a)
                  ? a.push(c)
                  : a !== t
                    ? (a = [a, c])
            ...
            g
              ? (P === t && C.initialize(), C.invoke(f))
              : (P !== t && P.invoke('destroy'), C.initialize());
        }),
        a !== t ? a : this
      );
    }),
