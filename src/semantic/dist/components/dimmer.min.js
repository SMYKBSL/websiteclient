      ...
      return (
        a.each(function() {
          var u,
            ...
            f,
            g = e.isPlainObject(i)
              ? e.extend(!0, {}, e.fn.dimmer.settings, i)
          ...
          (f = {
            preinitialize: function() {
              f.is.dimmer()
                ...
                : ((l = D),
                  (u = f.has.dimmer()
                    ? g.dimmerName
                      ? l.find(p.dimmer).filter('.' + g.dimmerName)
                      : l.find(p.dimmer)
                    : f.create()),
                  f.set.variation());
            },
            ...
            bind: {
              events: function() {
                'hover' == g.on
                  ? l.on('mouseenter' + y, f.show).on('mouseleave' + y, f.hide)
                  : 'click' == g.on && l.on(S + y, f.toggle),
                  f.is.page() &&
                    (f.debug('Setting as a page dimmer', l),
                    f.set.pageDimmer()),
                  f.is.closable() &&
            ...
            event: {
              click: function(i) {
                f.verbose('Determining if event occured on dimmer', i),
                  (0 === u.find(i.target).length ||
                    e(i.target).is(p.content)) &&
                    (f.hide(), i.stopImmediatePropagation());
              }
            },
            ...
            create: function() {
              ...
              return (
                g.dimmerName &&
                  (f.debug('Creating named dimmer', g.dimmerName),
            ...
            show: function(i) {
              (i = e.isFunction(i) ? i : function() {}),
                ...
                (f.is.dimmed() && !f.is.animating()) || !f.is.enabled()
                  ...
                  : (f.animate.show(i), g.onShow.call(T), g.onChange.call(T));
            },
            hide: function(i) {
              (i = e.isFunction(i) ? i : function() {}),
            ...
            has: {
              dimmer: function() {
                return g.dimmerName
                  ? D.find(p.dimmer).filter('.' + g.dimmerName).length > 0
                  : D.find(p.dimmer).length > 0;
              }
            },
            is: {
              active: function() {
            ...
            setting: function(i, n) {
              if ((f.debug('Changing setting', i, n), e.isPlainObject(i)))
                e.extend(!0, g, i);
              else {
                if (n === t) return g[i];
            ...
            f.preinitialize(),
            d
              ? (N === t && f.initialize(), f.invoke(m))
              : (N !== t && N.invoke('destroy'), f.initialize());
        }),
        o !== t ? o : this
      );
    }),
