...
(function($, window, document, undefined) {
        ...
        add: {
          id: function(id) {
            var currentIDs = module.retrieve(settings.key.ids),
              newIDs =
                currentIDs === undefined || currentIDs === ''
                  ? id
                  : currentIDs + settings.delimiter + id;
            if (module.has.visited(id)) {
              module.debug(
                'Unique content already visited, not adding visit',
                id,
                currentIDs
              );
          ...
          display: function(selector) {
            ...
            if ($element.length > 0 && !$.isWindow($element[0])) {
              ...
              $displays =
                $displays.length > 0 ? $displays.add($element) : $element;
            }
          }
        },
          ...
          limit: function(value) {
            ...
            if (settings.limit) {
              if (value >= settings.limit) {
                module.debug(
                  'Pages viewed exceeded limit, firing callback',
                  value,
                  settings.limit
                );
  ...
  $.fn.visit.settings = {
    ...
    error: {
      ...
      missingPersist:
        'Using the persist setting requires the inclusion of PersistJS',
      noCookieStorage:
        'The default storage cookie requires $.cookie to be included.'
    }
  };
