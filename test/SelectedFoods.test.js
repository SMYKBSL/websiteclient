...
describe('SelectedFoods', () => {
  ...
  describe('when prop `foods` is blank', () => {
    beforeEach(() => {
      wrapper = shallow(<SelectedFoods foods={[]} />);
    });
  ...
  describe('when `foods` is populated', () => {
    ...
    const totalKcals = foods
      .reduce((memo, f) => parseFloat(f.kcal) + memo, 0.0)
      .toFixed(2);

    beforeEach(() => {
      wrapper = shallow(<SelectedFoods foods={foods} />);
    });

    it('should display the foods in the table', () => {
      expect(wrapper.find('tbody tr').length).toEqual(foods.length);
    });
